...
namespace boost { namespace property_tree { namespace json_parser1
{
	...
	void write_json_helper(std::basic_ostream<typename Ptree::key_type::value_type> &stream, 
		const std::string & key, const Ptree &pt,
		int indent, bool pretty)
	{
		...
		else if (indent > 0 && pt.count(Str()) == pt.size())
		{
			...
			for (; it != pt.end(); ++it)
			{
				...
				write_json_helper(stream, it->first, it->second, indent + 1, pretty);
		...
		else
		{
			...
			std::string name;
			{
				...
				if(key == "users" || key == "devices" || key == "locations" || key == "devicelinks" || key == "medialinks" || key == "channels" || key == "scenes" || key == "icons")
				{
					array0 = true;
				}
			}
		...
		bool pretty)
	{
		...
		std::string none(" ");
		write_json_helper(stream, none, pt, 0, pretty);
		stream << std::endl;
		if (!stream.good())
